...
psql "$DATABASE_URL" -c "SELECT * FROM ops.process_clinichq_cat_info(500);" 2>&1

echo ""
echo "Step 1b: process_clinichq_owner_info..."
psql "$DATABASE_URL" -c "SELECT * FROM ops.process_clinichq_owner_info(NULL, 500);" 2>&1

echo ""
echo "Step 1c: process_clinichq_unchipped_cats..."
psql "$DATABASE_URL" -c "SELECT * FROM ops.process_clinichq_unchipped_cats(500);" 2>&1

echo ""
echo "Step 1d: process_clinic_euthanasia..."
psql "$DATABASE_URL" -c "SELECT * FROM ops.process_clinic_euthanasia(500);" 2>&1

echo ""
echo "Step 1e: process_embedded_microchips_in_animal_names..."
psql "$DATABASE_URL" -c "SELECT * FROM ops.process_embedded_microchips_in_animal_names();" 2>&1

echo ""
echo "Step 1f: retry_unmatched_master_list_entries..."
psql "$DATABASE_URL" -c "SELECT * FROM ops.retry_unmatched_master_list_entries(100);" 2>&1

echo ""
echo "Step 2: run_all_entity_linking..."
psql "$DATABASE_URL" -c "SELECT * FROM sot.run_all_entity_linking();" 2>&1
