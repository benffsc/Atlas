...
# Create mapping file (v1_cat_id|v2_cat_id)
cat > "$TEMP_DIR/mapping.csv" << 'MAPPING'
...
echo "Loaded $(wc -l < "$TEMP_DIR/mapping.csv") cat mappings"
echo ""

# Function to lookup V2 cat_id
lookup_v2_id() {
  grep "^$1|" "$TEMP_DIR/mapping.csv" | cut -d'|' -f2
}

# Get photos from V1 database
echo "1. Getting V1 photos from database..."
/opt/homebrew/opt/libpq/bin/psql "$V1_DB" -t -A -c "
SELECT name FROM storage.objects
WHERE bucket_id = 'request-media'
  AND name LIKE 'cats/%'
ORDER BY name
" > "$TEMP_DIR/v1_photos.txt"

total_photos=$(wc -l < "$TEMP_DIR/v1_photos.txt")
echo "   Found $total_photos photos in V1"
echo ""
...
while IFS= read -r v1_path; do
  # Extract cat_id and filename
  v1_cat_id=$(echo "$v1_path" | cut -d'/' -f2)
  filename=$(echo "$v1_path" | cut -d'/' -f3)

  # Look up V2 cat_id
  v2_cat_id=$(lookup_v2_id "$v1_cat_id")

  if [ -z "$v2_cat_id" ]; then
    echo "   SKIP: No V2 mapping for $v1_cat_id"
    skipped=$((skipped + 1))
    continue
  fi

  v2_path="cats/$v2_cat_id/$filename"
  local_file="$TEMP_DIR/$filename"

  # Download from V1 using public URL
  download_url="$V1_SUPABASE/storage/v1/object/public/request-media/$v1_path"
  http_code=$(curl -s -w "%{http_code}" -o "$local_file" "$download_url")

  if [ "$http_code" != "200" ]; then
    echo "   ERROR: Failed to download $v1_path (HTTP $http_code)"
    errors=$((errors + 1))
    continue
  fi
  ...
  upload_code=$(curl -s -w "%{http_code}" -o /dev/null \
    -X POST \
    -H "Authorization: Bearer $V2_KEY" \
    -H "Content-Type: $content_type" \
    --data-binary "@$local_file" \
    "$V2_SUPABASE/storage/v1/object/request-media/$v2_path")

  if [ "$upload_code" == "200" ] || [ "$upload_code" == "201" ]; then
    echo "   ✓ $v1_cat_id → $v2_path"
    migrated=$((migrated + 1))
  elif [ "$upload_code" == "400" ] || [ "$upload_code" == "409" ]; then
    # 400 = "Duplicate" in Supabase storage, 409 = Conflict
    skipped=$((skipped + 1))
  else
    echo "   ERROR: Failed to upload $v2_path (HTTP $upload_code)"
    errors=$((errors + 1))
  fi
