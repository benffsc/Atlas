...
console.log("Clinic Appointment Attribute Extraction");
console.log("=".repeat(60));
    ...
    for (const appt of appointments) {
      ...
      if ((!entityTypeFilter || entityTypeFilter === "cat") && appt.cat_id) {
        ...
        if (result.extractions.length > 0) {
          ...
          console.log(
            `[${recordsProcessed}] Cat ${appt.cat_name || appt.cat_id.slice(0, 8)}: ` +
      ...
      if ((!entityTypeFilter || entityTypeFilter === "place") && appt.place_id) {
        ...
        if (result.extractions.length > 0) {
          ...
          console.log(
            `[${recordsProcessed}] Place: ${result.extractions.map((e) => e.attribute_key).join(", ")}`
          );
        }
      }
