-- MIG_2908: Auto-Detect Unofficial Trappers
--
-- FFS-449: Creates queue_unofficial_trapper_candidates() function and adds
-- it as Step 7 to run_all_entity_linking().
--
-- After every entity linking run, we detect potential Tier 3 trappers from
-- data patterns and queue them in ops.data_quality_review_queue for staff review.
--
-- Created: 2026-03-11

\echo ''
\echo '=============================================='
\echo '  MIG_2908: Auto-Detect Unofficial Trappers'
\echo '  FFS-449'
\echo '=============================================='
\echo ''

-- ============================================================================
-- 1. CREATE QUEUE FUNCTION
-- ============================================================================

\echo '1. Creating sot.queue_unofficial_trapper_candidates()...'

CREATE OR REPLACE FUNCTION sot.queue_unofficial_trapper_candidates(
...
RETURNS TABLE (
  ...
  candidates_already_pending INT
)
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  v_rec RECORD;
BEGIN
  -- Wrap detect_unofficial_trappers() and queue results for staff review.
  -- Non-destructive: only creates pending review items, never auto-classifies.

  FOR v_rec IN
    SELECT * FROM sot.detect_unofficial_trappers(p_min_appointments, p_min_unique_places)
  LOOP
    v_found := v_found + 1;

    -- Check if already pending in review queue
    IF EXISTS (
      SELECT 1 FROM ops.data_quality_review_queue
      WHERE entity_id = v_rec.person_id
        AND issue_type = 'unofficial_trapper_candidate'
        AND status = 'pending'
    ) THEN
      v_already_pending := v_already_pending + 1;
      CONTINUE;
    END IF;

    -- Queue for review
    INSERT INTO ops.data_quality_review_queue (
      entity_type, entity_id, issue_type, suggested_action, details, status
    ) VALUES (
      'person',
      v_rec.person_id,
      'unofficial_trapper_candidate',
      'Review appointment patterns. If confirmed trapper, add to trapper_profiles and trapper_service_places.',
      jsonb_build_object(
        'person_name', v_rec.person_name,
        'appointment_count', v_rec.appointment_count,
        'unique_places', v_rec.unique_places,
        'evidence', v_rec.evidence,
        'detected_at', NOW()
      ),
      'pending'
    )
    ON CONFLICT (entity_id, issue_type) WHERE status = 'pending'
    DO NOTHING;
  ...
  candidates_queued := v_queued;
  candidates_already_pending := v_already_pending;
  RETURN NEXT;
END;
$$;

COMMENT ON FUNCTION sot.queue_unofficial_trapper_candidates IS
'FFS-449: Wraps detect_unofficial_trappers() and queues candidates in
ops.data_quality_review_queue for staff review. Non-destructive — never
auto-classifies. Called as Step 7 of run_all_entity_linking().';

\echo '   Created sot.queue_unofficial_trapper_candidates()'

-- ============================================================================
-- 2. ADD STEP 7 TO run_all_entity_linking()
-- ============================================================================

\echo ''
\echo '2. Adding Step 7 to run_all_entity_linking()...'
...
BEGIN
    ...
    BEGIN
        ...
        SELECT removed INTO v_stale_removed
        FROM sot.cleanup_stale_request_cat_links();

        -- 4b: Create fresh links via place family + attribution window
        SELECT before_request, during_request, grace_period
        INTO v_before, v_during, v_grace
        FROM sot.link_cats_to_requests_attribution();

        v_result := v_result || jsonb_build_object(
            'step4_stale_removed', COALESCE(v_stale_removed, 0),
            'step4_cats_linked_to_requests', COALESCE(v_before, 0) + COALESCE(v_during, 0) + COALESCE(v_grace, 0),
    ...
    -- ========================================================================
    -- STEP 7 (NEW/MIG_2908): Queue unofficial trapper candidates
    -- ========================================================================
...
'V2/MIG_2908: Master orchestrator for entity linking pipeline.
Complete pipeline with 7 steps (MIG_2867 had 6, MIG_2908 adds step 7).
...
\echo '   Added Step 7 to run_all_entity_linking()'
...
WHERE n.nspname = 'sot'
  AND p.proname IN ('queue_unofficial_trapper_candidates', 'run_all_entity_linking');

-- Test queue function (dry run)
\echo ''
\echo 'Unofficial trapper candidates (preview):'
SELECT * FROM sot.detect_unofficial_trappers(3, 2) LIMIT 5;

\echo ''
\echo '=============================================='
\echo '  MIG_2908 COMPLETE'
\echo '=============================================='
