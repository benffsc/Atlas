-- MIG_2522: Enhanced Comprehensive Place Lookup for Tippy
-- Date: 2026-02-26
--
-- Purpose: Create a truly comprehensive place lookup that returns:
--   1. Place info (address, type, coordinates)
--   2. People connected (with roles and contact info)
--   3. Cat statistics (total, altered, unaltered, alteration rate)
--   4. Disease testing results
--   5. Request history
--   6. Related places (same owner/operation)
--   7. Colony status assessment ("under control", "needs attention", etc.)

\echo ''
\echo '=============================================='
\echo '  MIG_2522: Enhanced Comprehensive Place Lookup'
\echo '=============================================='
\echo ''
    ...
    SELECT JSONB_BUILD_OBJECT(
        ...
        'people', (
            SELECT COALESCE(JSONB_AGG(JSONB_BUILD_OBJECT(
                ...
                'email', (SELECT pi.id_value_raw FROM sot.person_identifiers pi
                         WHERE pi.person_id = pe.person_id AND pi.id_type = 'email'
                         AND pi.confidence >= 0.5 LIMIT 1),
                'phone', (SELECT pi.id_value_raw FROM sot.person_identifiers pi
        ...
        'related_places', (
            -- Find other places connected to the same people
            SELECT COALESCE(JSONB_AGG(JSONB_BUILD_OBJECT(
...
'Comprehensive place report for Tippy with people, cats, diseases, requests, and status assessment';

-- Also create a simpler summary function
...
'Human-readable place summary for Tippy';
...
\echo 'Testing tippy_place_full_report...'
SELECT jsonb_pretty(ops.tippy_place_full_report('1170 Walker'));

\echo ''
\echo 'Testing tippy_place_summary...'
