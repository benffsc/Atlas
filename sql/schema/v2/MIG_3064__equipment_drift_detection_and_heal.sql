-- MIG_3064: Equipment drift detection + self-heal + Airtable cron retirement
--
...
--
-- Run with:
--   psql $DATABASE_URL -f sql/schema/v2/MIG_3064__equipment_drift_detection_and_heal.sql
...
COMMENT ON VIEW ops.v_equipment_drift IS
  'Compares each equipment row against its latest atlas_ui event. Used for drift detection — any non-empty drift_reasons array indicates the equipment row state disagrees with what the kiosk recorded. See MIG_3064.';
COMMENT ON VIEW ops.v_equipment_drift_active IS
  'Filtered view of v_equipment_drift showing only rows with active drift. Should be empty in steady state. Any rows here = silent data corruption that needs healing via ops.heal_equipment_from_events(). See MIG_3064.';
...
COMMENT ON FUNCTION ops.heal_equipment_from_events(UUID) IS
  'Recomputes ops.equipment row state from the canonical equipment_events log. Returns BEFORE/AFTER state if anything was healed, or empty if no drift. The events table is the source of truth; this function exists because the row cache can drift if a non-trigger writer (Airtable cron, admin edit, migration) clobbers kiosk-owned columns. See MIG_3064.';
