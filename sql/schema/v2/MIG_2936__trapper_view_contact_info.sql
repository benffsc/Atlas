-- MIG_2936: Add email/phone to v_trapper_full_stats + fix role filter (FFS-541, FFS-537)
-- Date: 2026-03-13
-- Changes:
--   1. Add sot.get_email() / sot.get_phone() columns for contact info
--   2. Fix WHERE clause: role = 'trapper' only (was including staff/coordinator)
--   3. Recreate aggregate view to stay in sync

-- Drop and recreate both views (aggregate depends on full stats)
