-- MIG_2890: Fix Entity Linking Errors Blocking Ingest (FFS-387)
--
-- PROBLEM: Three bugs in ingest pipeline entity linking:
-- 1. sot.run_cat_place_linking() assigns TABLE result to INTEGER
--    (V2 link_cats_to_places() returns 4 columns, V1 wrapper expected scalar)
-- 2. ops.detect_owner_changes() references pi_email.id_value (column renamed to id_value_raw in V2)
-- 3. Step 4d owner change detection had no try-catch, crashing owner_info processing
--
-- Created: 2026-03-09
-- Fixes: FFS-387
...
\echo ''
\echo '=============================================='
\echo '  MIG_2890: Fix Entity Linking Errors'
\echo '=============================================='
\echo ''

-- ============================================================================
-- 1. FIX sot.run_cat_place_linking() — V1 wrapper calling V2 function
-- ============================================================================

\echo '1. Fixing sot.run_cat_place_linking()...'

CREATE OR REPLACE FUNCTION sot.run_cat_place_linking()
RETURNS TABLE(cats_linked integer, places_involved integer)
LANGUAGE plpgsql AS $function$
DECLARE
  v_appointment_linked INTEGER;
BEGIN
  -- Link cats to places via appointment inferred_place_id
  SELECT COALESCE(SUM(r.cats_linked), 0) INTO v_appointment_linked
  FROM sot.link_cats_to_appointment_places() r;

  -- Link cats to places via person chain (V2 returns TABLE, not scalar)
  SELECT r.cats_linked_home + r.cats_linked_appointment, r.total_edges
  INTO cats_linked, places_involved
  FROM sot.link_cats_to_places() r;

  cats_linked := COALESCE(cats_linked, 0) + v_appointment_linked;
  RETURN NEXT;
END;
$function$;

COMMENT ON FUNCTION sot.run_cat_place_linking IS
'V1 compatibility wrapper. Calls V2 link_cats_to_appointment_places() and link_cats_to_places().
Fixed in MIG_2890: link_cats_to_places() returns TABLE not scalar.';

\echo '   Fixed sot.run_cat_place_linking()'

-- ============================================================================
-- 2. FIX ops.detect_owner_changes() — id_value → id_value_raw
-- ============================================================================

\echo ''
\echo '2. Fixing ops.detect_owner_changes()...'
...
BEGIN
    ...
    LOOP
        -- Get existing person info (FIXED: id_value_raw not id_value)
        SELECT
            p.person_id,
            p.display_name,
        ...
        INSERT INTO ops.review_queue (
            ...
            old_person_id, new_person_id, match_confidence, change_context,
            cats_affected, source_system, source_record_id
        ) VALUES (
            'person',
            v_record.existing_person_id,
            ...
            END,
            CASE v_classification.change_type
                WHEN 'ownership_transfer' THEN 10
                WHEN 'new_entity' THEN 10
                WHEN 'household_member' THEN 5
                ELSE 1
            END,
            'pending',
...
'Detects owner changes in ClinicHQ batch uploads.
Compares incoming owner data against existing person identifiers.
Fixed in MIG_2890: id_value → id_value_raw (column rename in V2).';

\echo '   Fixed ops.detect_owner_changes()'
...
\echo ''
\echo '3a. Test run_cat_place_linking():'
SELECT * FROM sot.run_cat_place_linking();

\echo ''
\echo '=============================================='
\echo '  MIG_2890 Complete'
\echo '=============================================='
\echo ''
