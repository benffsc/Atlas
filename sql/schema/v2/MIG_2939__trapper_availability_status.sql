-- MIG_2939: Add availability_status to trapper_profiles
-- FFS-543: Trapper availability for dispatch workflow
--
-- Tracks whether a trapper is currently available to take new assignments.
-- Values: available (default), busy, on_leave

BEGIN;

\echo 'MIG_2939: Adding availability_status to trapper_profiles'

ALTER TABLE sot.trapper_profiles
  ADD COLUMN IF NOT EXISTS availability_status TEXT NOT NULL DEFAULT 'available'
  CHECK (availability_status IN ('available', 'busy', 'on_leave'));

-- Rebuild views to include availability_status
...
SELECT
  ...
  -- Availability breakdown (FFS-543)
...
\echo 'MIG_2939: Availability status added'

-- Verify
SELECT availability_status, COUNT(*) FROM ops.v_trapper_full_stats GROUP BY 1;
