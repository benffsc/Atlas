-- MIG_2883: Extract ClinicHQ animal IDs from animal_heading_raw (FFS-374)
--
-- Daniel's scraper captured the raw heading text which contains the ClinicHQ
-- animal ID in parentheses: "Lotus 981020053774577 (24-3487)"
-- The scraper's name parser split out the cat name but didn't extract the ID.
--
-- This migration:
--   1. Adds extracted_clinichq_id column to source.clinichq_scrape
--   2. Populates it via regex from animal_heading_raw: (XX-XXXX) pattern
--   3. Updates enrichment views to use clinichq_id as additional matching tier
--
-- Impact: 20,480 records have extractable IDs (19,083 match to existing cats)
--   - 480 new cat matches for records previously unmatched
--   - Improves cat notes view and appointment enrichment coverage
--
...
-- =============================================================================
-- Step 1: Add extracted_clinichq_id column
-- =============================================================================

ALTER TABLE source.clinichq_scrape
    ADD COLUMN IF NOT EXISTS extracted_clinichq_id TEXT;

COMMENT ON COLUMN source.clinichq_scrape.extracted_clinichq_id IS
    'ClinicHQ animal ID (XX-XXXX format) extracted from animal_heading_raw parentheses (MIG_2883)';

CREATE INDEX IF NOT EXISTS idx_clinichq_scrape_extracted_id
    ON source.clinichq_scrape(extracted_clinichq_id)
    WHERE extracted_clinichq_id IS NOT NULL;

-- =============================================================================
-- Step 2: Extract IDs from heading
-- =============================================================================

UPDATE source.clinichq_scrape
SET extracted_clinichq_id = (regexp_match(animal_heading_raw, '\(([0-9]{1,3}-[0-9]{1,5})\)'))[1]
WHERE animal_heading_raw ~ '\([0-9]{1,3}-[0-9]{1,5}\)'
  AND extracted_clinichq_id IS NULL;

-- =============================================================================
-- Step 3: Rebuild enrichment views with clinichq_id tier
-- =============================================================================

-- Drop views that depend on these (scrape-only views, safe)
...
-- 3a: Appointment enrichment — add Tier 4: clinichq_id matching
...
COMMENT ON VIEW ops.v_scrape_appointment_enrichment IS
    'Joins scrape to appointments via tiered matching: microchip+date > client+name+date > client+date solo > clinichq_id (MIG_2883). ~70% appointment match, ~85% cat match.';

-- 3b: Cat notes view — add clinichq_id matching tier
...
COMMENT ON VIEW ops.v_scrape_cat_notes IS
    'Scrape notes joined to sot.cats via microchip (primary) > clinichq_id > animal name (MIG_2883)';

-- 3c: Mortality detail view — add clinichq_id matching
...
COMMENT ON VIEW ops.v_scrape_mortality_detail IS
    'Parses deceased labels — timing and cause. Cat match via microchip > clinichq_id (MIG_2883)';

-- 3d: Trapper attribution view — add clinichq_id matching
...
COMMENT ON VIEW ops.v_scrape_trapper_attribution IS
    'Trapper attribution — cat match via microchip > clinichq_id (MIG_2883)';
...
DECLARE
    ...
    v_cat_notes_by_type RECORD;
BEGIN
    SELECT COUNT(*) INTO v_extracted FROM source.clinichq_scrape WHERE extracted_clinichq_id IS NOT NULL;
    SELECT COUNT(*) INTO v_total FROM source.clinichq_scrape;

    RAISE NOTICE 'MIG_2883: ClinicHQ animal ID extraction complete';
    RAISE NOTICE '  IDs extracted from heading: % of % (%.1f%%)', v_extracted, v_total, (v_extracted::numeric / v_total * 100);
    RAISE NOTICE '';
    ...
    RAISE NOTICE '';
    RAISE NOTICE '  Cat notes match rates:';
    FOR v_cat_notes_by_type IN
        SELECT match_type, COUNT(*) AS ct
        FROM ops.v_scrape_cat_notes
        GROUP BY 1 ORDER BY ct DESC
    LOOP
        RAISE NOTICE '    %: %', v_cat_notes_by_type.match_type, v_cat_notes_by_type.ct;
    END LOOP;
END $$;
