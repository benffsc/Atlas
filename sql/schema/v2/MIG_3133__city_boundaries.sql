...
-- Helper function: count TNR'd cats within a city's official limits
...
LANGUAGE SQL STABLE AS $$
  SELECT
    EXTRACT(YEAR FROM COALESCE(
      (SELECT MIN(a.appointment_date) FROM ops.appointments a WHERE a.cat_id = c.cat_id),
      cp.created_at
    ))::INT AS year,
    COUNT(*) AS total_tnr,
    COUNT(*) FILTER (WHERE c.altered_status = 'spayed') AS spayed,
    COUNT(*) FILTER (WHERE c.altered_status = 'neutered') AS neutered
  FROM sot.cats c
