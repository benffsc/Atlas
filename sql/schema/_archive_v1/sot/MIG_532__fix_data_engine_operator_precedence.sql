\echo ''
\echo '=============================================='
\echo 'MIG_532: Fix Data Engine Operator Precedence Bug'
\echo '=============================================='
\echo ''
\echo 'CRITICAL BUG FIX: The data_fixing_patterns check was incorrectly'
\echo 'rejecting ALL names when ANY is_organization pattern existed,'
\echo 'regardless of whether the name actually matched the pattern.'
\echo ''
\echo 'Root cause: Missing parentheses in WHERE clause caused:'
\echo '  WHERE is_organization = TRUE OR is_garbage = TRUE AND ...'
\echo 'to evaluate as:'
\echo '  WHERE (is_organization = TRUE) OR (is_garbage = TRUE AND ...)'
\echo ''
\echo 'Fix: Add parentheses to make it:'
\echo '  WHERE (is_organization = TRUE OR is_garbage = TRUE) AND ...'
\echo ''

-- ============================================================================
-- FIX data_engine_resolve_identity() OPERATOR PRECEDENCE
-- ============================================================================

\echo 'Fixing data_engine_resolve_identity operator precedence...'
...
BEGIN
    ...
    -- =========================================================================
    -- CHECK FOR ORGANIZATION REPRESENTATIVE MAPPING
    ...
    IF v_display_name IS NOT NULL AND v_display_name != '' THEN
        ...
        -- FIXED: Added parentheses around (is_organization OR is_garbage) to fix operator precedence
        ELSIF EXISTS (
...
'Identity resolution with operator precedence bug fix (MIG_532):
- FIXED: data_fixing_patterns check now correctly requires BOTH
  (is_organization OR is_garbage) AND pattern match
...
- decision_type: "org_representative" for mapped orgs';
...
\echo '=============================================='
\echo ''
\echo 'CRITICAL BUG FIX: Fixed SQL operator precedence in the'
\echo 'data_fixing_patterns check within data_engine_resolve_identity().'
\echo ''
\echo 'Before: WHERE is_organization = TRUE OR is_garbage = TRUE AND ...'
\echo '        (matched ALL names when ANY is_organization pattern existed)'
\echo ''
\echo 'After:  WHERE (is_organization = TRUE OR is_garbage = TRUE) AND ...'
\echo '        (correctly requires the pattern to actually match the name)'
\echo ''
\echo 'Test with:'
\echo '  SELECT * FROM trapper.data_engine_resolve_identity('
\echo '    ''test@example.com'', ''+14155551234'', ''Chris'', ''Andersen'','
