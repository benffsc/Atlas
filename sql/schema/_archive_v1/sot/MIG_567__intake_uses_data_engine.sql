\echo '=== MIG_567: Update create_person_from_intake to use Data Engine ==='
\echo 'This ensures intake submissions go through proper identity resolution'

-- The current create_person_from_intake (from MIG_566) does manual email/phone lookup
-- This bypasses the Data Engine's multi-signal scoring and can create duplicates.
-- Fix: Use find_or_create_person() which delegates to Data Engine internally.
...
BEGIN
  ...
  ELSE
    -- Normalize email and phone (for blacklist check only)
    ...
    -- Build requester address for identity resolution context
    ...
    -- This handles:
    --   1. Multi-signal weighted scoring (email, phone, name, address)
    --   2. Household modeling (multiple people at same address)
    --   3. Configurable matching rules and thresholds
    --   4. Review queue for uncertain matches
    --   5. Full audit trail of matching decisions
    v_person_id := trapper.find_or_create_person(
      p_email := v_norm_email,
      p_phone := v_norm_phone,
      p_first_name := v_sub.first_name,
      p_last_name := v_sub.last_name,
      p_address := v_requester_address,
      p_source_system := 'web_intake'
    );

    -- Update submission with matched/created person
    IF v_person_id IS NOT NULL THEN
      UPDATE trapper.web_intake_submissions
      SET matched_person_id = v_person_id
      WHERE submission_id = p_submission_id;

      RAISE NOTICE 'Matched/created person % for submission % via Data Engine', v_person_id, p_submission_id;
    END IF;
  END IF;
  ...
  -- Build full requester address (reuse if already built)
  IF v_requester_address IS NULL THEN
    v_requester_address := NULLIF(TRIM(
      COALESCE(v_sub.requester_address, '') ||
      CASE WHEN v_sub.requester_city IS NOT NULL THEN ', ' || v_sub.requester_city ELSE '' END ||
      CASE WHEN v_sub.requester_zip IS NOT NULL THEN ' ' || v_sub.requester_zip ELSE '' END
...
\echo 'Updated create_person_from_intake to use Data Engine via find_or_create_person()'
\echo 'Benefits:'
\echo '  - Multi-signal weighted scoring (email, phone, name, address)'
\echo '  - Household modeling for people at same address'
\echo '  - Configurable matching rules in database'
\echo '  - Full audit trail in data_engine_match_decisions'
