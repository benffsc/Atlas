-- MIG_150__search_function_alias_support.sql
-- Update search_unified function to search person aliases
--
-- MANUAL APPLY:
--   export $(cat .env | grep -v '^#' | xargs)
--   psql "$DATABASE_URL" -f sql/schema/sot/MIG_150__search_function_alias_support.sql

\echo ''
\echo 'Updating search_unified function with alias support...'
...
'Unified search across cats, people (including aliases), and places. Returns ranked results.';
...
SELECT
    ...
    match_reason,
    score,
    metadata->>'alias_count' as alias_count,
    metadata->>'matched_alias' as matched_alias
FROM trapper.search_unified('cal eggs', NULL, 10, 0);

SELECT 'MIG_150 Complete' AS status;
