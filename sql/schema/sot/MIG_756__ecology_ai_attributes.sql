\echo '=== MIG_756: Incorporate AI-Extracted Population Attributes into Ecology Stats ==='
\echo 'Updates v_place_ecology_stats to use is_recapture, eartip_count_observed, unfixed_count_observed from entity_attributes'

-- ============================================================================
-- UPDATE v_place_ecology_stats TO INCLUDE AI-EXTRACTED ATTRIBUTES
-- ============================================================================
-- This migration adds:
-- 1. AI-extracted eartip/unfixed counts from entity_attributes
-- 2. Recapture counts from cat attributes (is_recapture = true)
-- 3. Combined counts for Chapman calculation (manual + AI-extracted)
...
ai_eartip_observations AS (
  SELECT
    ...
    SUM((attribute_value)::int) AS ai_eartips_seen
...
ai_unfixed_observations AS (
  SELECT
    entity_id AS place_id,
    SUM((attribute_value)::int) AS ai_unfixed_seen
  FROM trapper.entity_attributes
  WHERE entity_type = 'place'
...
recapture_counts AS (
  ...
  WHERE ea.entity_type = 'cat'
    ...
    AND ea.attribute_value::boolean = true
...
SELECT
  ...
  -- Recapture data (NEW - from AI extraction)
  ...
  -- Estimation method (now shows ai_enhanced when AI data present)
  CASE
...
- NEW: Incorporates eartip_count_observed and unfixed_count_observed from entity_attributes
- NEW: recapture_count from is_recapture cat attribute';

-- ============================================================================
-- CREATE SUMMARY VIEW FOR AI POPULATION DATA COVERAGE
-- ============================================================================
...
WHERE entity_type = 'cat'
  ...
  AND attribute_value::boolean = true
  AND superseded_at IS NULL
...
\echo ''
\echo '=== MIG_756 Complete ==='
\echo 'Updated v_place_ecology_stats to incorporate AI-extracted population attributes:'
\echo '  - eartip_count_observed and unfixed_count_observed now feed Chapman calculation'
\echo '  - is_recapture counts shown per place'
\echo '  - estimation_method shows mark_resight_ai_enhanced when AI data present'
\echo 'Created v_ai_population_data_summary for coverage monitoring'
\echo ''
