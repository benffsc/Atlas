...
export function CatCard({ cat, onClick, compact = false, showAddress = true, showOwner = false, onUpdateClinicDayNumber }: CatCardProps) {
  ...
  // Inline edit state for clinic_day_number
  const [isEditingNumber, setIsEditingNumber] = useState(false);
  const [editNumberValue, setEditNumberValue] = useState<string>(
    cat.clinic_day_number?.toString() || ""
  );
  ...
  // Save clinic day number
  const handleSaveNumber = async () => {
    if (!onUpdateClinicDayNumber || !cat.appointment_id) {
      setIsEditingNumber(false);
      return;
    }

    const newValue = editNumberValue.trim() === "" ? null : parseInt(editNumberValue, 10);
    ...
    try {
      await onUpdateClinicDayNumber(cat.appointment_id, newValue);
      setIsEditingNumber(false);
    } catch (err) {
  ...
  return (
    ...
    >
      {/* Clinic Day Number Badge (editable if callback provided) */}
      {(cat.clinic_day_number || (onUpdateClinicDayNumber && cat.appointment_id)) && (
        <div
          style={{
            position: "absolute",
            top: compact ? "4px" : "8px",
        ...
        >
          {isEditingNumber ? (
            ...
            >
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape") {
                    setEditNumberValue(cat.clinic_day_number?.toString() || "");
                    setIsEditingNumber(false);
                  }
                }}
                autoFocus
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: "0.7rem",
                  fontWeight: 600,
                  cursor: isSavingNumber ? "wait" : "pointer",
                }}
              >
                {isSavingNumber ? "..." : "✓"}
              </button>
            ...
            <div
              ...
              style={{
                background: cat.clinic_day_number ? "var(--primary)" : "var(--muted)",
                color: cat.clinic_day_number ? "var(--primary-foreground)" : "var(--foreground)",
                padding: compact ? "2px 6px" : "4px 10px",
                borderRadius: "6px",
                fontSize: compact ? "0.7rem" : "0.8rem",
                fontWeight: 700,
                boxShadow: "0 2px 4px rgba(0,0,0,0.2)",
                cursor: onUpdateClinicDayNumber && cat.appointment_id ? "pointer" : "default",
                border: onUpdateClinicDayNumber && cat.appointment_id && !cat.clinic_day_number
                  ? "1px dashed var(--card-border)"
                  : "none",
              }}
              title={onUpdateClinicDayNumber && cat.appointment_id ? "Click to edit" : undefined}
            >
              {cat.clinic_day_number ? `#${cat.clinic_day_number}` : "#?"}
            </div>
          )}
        </div>
      )}
