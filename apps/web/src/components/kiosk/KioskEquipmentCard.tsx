...
function getActionButtonProps(action: string): {
  ...
  switch (action) {
    ...
    case "assign":
      return { variant: "primary", icon: "user-check" };
    case "check_in":
      return {
        variant: "secondary",
        icon: "log-in",
    ...
    case "correction":
      return { variant: "ghost", icon: "pencil" };
    case "note":
...
export function KioskEquipmentCard({ equipment, onAction }: KioskEquipmentCardProps) {
  ...
  const isOut = equipment.custody_status === "checked_out" || equipment.custody_status === "in_field" || equipment.custody_status === "assigned";
  ...
  return (
    ...
    >
      ...
      {/* Essential info — always visible: custodian + condition */}
      <div
        style={{
      ...
      >
        {isOut && (
          <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.9rem" }}>
            ...
            <span style={{ fontWeight: 500 }}>
              {equipment.custodian_name || equipment.current_holder_name || "Unknown"}
            </span>
          </div>
        )}
        <span
        ...
        {isOut && dueDate && (
          <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.85rem" }}>
            <Icon name={isOverdue ? "alert-circle" : "calendar"} size={14} color={isOverdue ? "var(--danger-text)" : "var(--muted)"} />
            <span style={{ color: isOverdue ? "var(--danger-text)" : "var(--text-secondary)", fontWeight: isOverdue ? 600 : 400 }}>
              {formatDate(dueDate)}
            </span>
          </div>
        )}
      ...
      {/* Expanded details — photo + full info grid */}
      {expanded && (
        <>
          {equipment.photo_url && (
            <div style={{
