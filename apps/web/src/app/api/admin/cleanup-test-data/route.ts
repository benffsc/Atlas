...
export async function POST() {
  ...
  const cleanupSteps = [
    { label: "cat_place_relationships", sql: "DELETE FROM sot.cat_place_relationships WHERE evidence_type = 'e2e_test'" },
    { label: "person_cat_relationships", sql: "DELETE FROM sot.person_cat_relationships WHERE evidence_type = 'e2e_test'" },
    { label: "person_place_relationships", sql: "DELETE FROM sot.person_place_relationships WHERE evidence_type = 'e2e_test'" },
