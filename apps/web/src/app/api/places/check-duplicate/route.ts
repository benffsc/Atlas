  ...
  try {
    ...
    const existingPlaces = await queryRows<PlaceRow>(
      `SELECT
        ...
        p.formatted_address,
        p.place_kind::TEXT,
        (SELECT COUNT(*) FROM sot.cat_place WHERE place_id = p.place_id AND COALESCE(presence_status, 'unknown') NOT IN ('departed', 'presumed_departed'))::INT as cat_count,
      ...
      ORDER BY
        (SELECT COUNT(*) FROM sot.cat_place WHERE place_id = p.place_id AND COALESCE(presence_status, 'unknown') NOT IN ('departed', 'presumed_departed')) +
